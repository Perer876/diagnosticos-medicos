    ...
    <x-slot name="header">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                ...
                <p class="text-subtitle text-muted">Agrega o actualiza tratamientos de la enfermedad en cuestion</p>
            </div>
    ...
    <section class="section row justify-content-center">
        <div class="card col-12 col-md-10 col-xl-8">
            ...
            <div class="card-content">
                <div class="card-body">
                    <form class="form" method="post" action="{{ route('enfermedades.update-tratamiento', $enfermedade) }}">
                        @method('PATCH')
                        @csrf                        
                        <div class="form-group">
                            <label for="tratamientosInput" class="sr-only">
                                Tratamientos
                            </label>
                            <select class="form-select" multiple="multiple" name="tratamientos[]" id="tratamientosInput"
                                x-data x-ref="tratamientos" x-init="
                                    new Choices($refs.tratamientos, {
                                ...
                                "
                                >
                                @foreach(\App\Models\Tratamiento::pluck('nombre', 'id') as $id => $nombre)
                                    <option value="{{ $id }}" @selected($enfermedade->tratamientos->contains($id))>
                                        {{ $nombre }}
                                    </option>
                                @endforeach
                            </select>
